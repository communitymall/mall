    ...
    export default {
        ...
        created() {
            if (_.has(this.$route.params, 'orderId')) {
                ...
                localStorage.setItem("checkOrderID",this.$route.params.orderId);
        ...
        methods: {
            ...
            pay() {
                ...
                orderPrepay({orderId: this.orderId, H5: 1}).then(res => {
                    var payParams = res.data.mweb_url;
                    window.location.href = payParams
                });

                this.$router.push("/order/payment/:fail")
            }
        },
